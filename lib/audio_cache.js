...
/**
 * Get all keys in the audio cache.
...
export async function getAllKeys() {
  try {
    ...
    console.error('Audio cache getAllKeys failed:', e);
    dbInstance = null;
    return [];
  }
}
...
/**
 * Get audio blob by raw key (not text+voiceId, but the actual store key).
 * Used by Drive sync to read cache entries directly.
 * @param {string} key
...
export async function getAudioByKey(key) {
  try {
    ...
    console.error('Audio cache getAudioByKey failed:', e);
    dbInstance = null;
    return null;
  }
}
...
/**
 * Save audio blob by raw key.
 * Used by Drive sync to import cache entries.
 * @param {string} key
 * @param {Blob} blob
 */
export async function saveAudioByKey(key, blob) {
  try {
    const db = await getDB();
    return new Promise((resolve, reject) => {
      const transaction = db.transaction([STORE_NAME], 'readwrite');
    ...
    console.error('Audio cache saveAudioByKey failed:', e);
    dbInstance = null;
    throw e;
  }
}
